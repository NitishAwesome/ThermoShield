...
def validate_canonical_weather(data: Dict[str, Any] | CanonicalWeatherObservation) -> CanonicalWeatherObservation:
    ...
    required = [
        'location', 'ward', 'latitude', 'longitude', 'timestamp',
        'temperature_c', 'relative_humidity_pct', 'wind_speed_mps', 'solar_radiation_wm2',
    ]
...
def load_demo_observations(dataset_path: Optional[str] = None, include_scenario: bool = False) -> List[Dict[str, Any]]:
...
def demo_observation_for_scenario(scenario: str, dataset_path: Optional[str] = None, include_scenario: bool = False) -> Dict[str, Any]:
    normalized = scenario.strip().upper()
    for record in load_demo_observations(dataset_path=dataset_path, include_scenario=include_scenario):
        if include_scenario:
            if record.get('scenario', '').upper() == normalized:
                return record
        else:
            # Re-read raw items once to match scenario while keeping default output lean.
            pass
    raw = json.loads((Path(dataset_path) if dataset_path else DATA_FILE).read_text(encoding='utf-8'))
    for item in raw:
