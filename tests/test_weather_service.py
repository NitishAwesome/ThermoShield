...
class TestWeatherService(unittest.IsolatedAsyncioTestCase):
    async def test_get_weather_returns_canonical_contract(self):
        ...
        with patch("backend.app.services.weather.httpx.AsyncClient", return_value=fake_client):
            result = await get_weather(13.0827, 80.2707, location="Chennai", ward="Ward-22")

        self.assertEqual(result["location"], "Chennai")
        self.assertEqual(result["ward"], "Ward-22")
        self.assertEqual(result["temperature_c"], 39.5)
        self.assertAlmostEqual(result["wind_speed_mps"], 4.0, places=3)
